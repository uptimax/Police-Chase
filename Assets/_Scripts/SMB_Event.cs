...
public class SMB_Event : StateMachineBehaviour
{
 [System.Serializable]
  ...
  private SMB_EventCurrator m_eventCurrator;

  public override void OnStateEnter(Animator animator, AnimatorStateInfo animatorStateInfo, int layerIndex) {
    m_hasParam = HasParameter(animator, m_motionTime);
    m_eventCurrator = animator.gameObject.GetComponentInChildren<SMB_EventCurrator>();
    m_totalFrames = GetTotalFrames(animator, layerIndex);

    m_normalizedTimeUncapped = animatorStateInfo.normalizedTime;
    m_normalizedTime = m_hasParam ? animator.GetFloat(m_motionTime) : GetNormalizedTime(animatorStateInfo);
  ...
  private bool HasParameter(Animator animator, string parameterName){
    if(string.IsNullOrEmpty(parameterName) || string.IsNullOrWhiteSpace(parameterName)) return false;
